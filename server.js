...
app.post('/start', async (req, res) => {
    ...
    try {
        // Create the WebRTC Go video room, PlayerStream, and MediaProcessors
        const room = await twilioClient.video.rooms.create({
            uniqueName: streamName,
            type: 'go'
        });

        // allow the livestreamed content to be sent to the audience
        ...
        const mediaProcessor = await twilioClient.media.mediaProcessor.create({
            // uses the Video Composer extension to compose the video feeds into a grid layout and send the content to the PlayerStreamer
        ...
        console.log('unable to create livestream');
...
/**
 * End a livestream
 * This endpoint takes the streamDetails from the request and sets the PlayerStreamer and MediaProcessor's statuses to ended
 */
...
*  - Create a new access token for the streamer 
*  - add videoGrant
*/
...
app.post('/audienceToken', async (req, res) => {
    // Generate random string for the identity
    const identity = crypto.randomBytes(20).toString('hex');

    try {
        // get first player streamer
        ...
        // Other wise create token with PlaybackGrant for livestream
        const token = new AccessToken(accountSid, apiKey, apiKeySecret);
